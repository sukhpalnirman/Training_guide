...
ResultSet rs=s.executeQuery("select * from admind where passw='"+request.getParameter("t1")+"' and aid="+session.getAttribute("aid"));
if(rs.next())
{
	s.execute("update admind set passw='"+request.getParameter("t2")+"' where aid="+session.getAttribute("aid"));

con.close();

response.sendRedirect("cpwa.jsp?p=1");
}
else
{

con.close();
response.sendRedirect("cpwa.jsp?p=2");
}
}
catch(Exception ex)
{
