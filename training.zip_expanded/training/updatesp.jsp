...
ResultSet rs=s.executeQuery("select * from studentd where passw='"+request.getParameter("t1")+"' and sname='"+session.getAttribute("nm")+"'");
if(rs.next())
{
	s.execute("update studentd set passw='"+request.getParameter("t2")+"' where sname='"+session.getAttribute("nm")+"'");

con.close();

response.sendRedirect("cpws.jsp?p=1");
}
else
{

con.close();
response.sendRedirect("cpws.jsp?p=2");
}
}
catch(Exception ex)
{
